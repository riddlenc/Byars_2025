...
```
These results were used in the manuscript as Supplementary Table 2
...
```
Two p-values used in manuscript
P<6.1e-10, ANOVA
P<8.1e-08, ANOVA
...
Values for use in the manuscript text
#summarize 786 mean and total activity data
Used this in the manuscript text
```{r}
activity_summary_786 <- master_786_outliers %>% 
...
activity_summary_786
```
For manuscript text:
Comparing day 26 to 51 females
5D goes from 6.16 to 0.18, 97% reduction
20D goes from 3.63 to 0.161, 95% reduction (w/o rounding up)
C goes from 5.66 to 0.22, 96% reduction

786 males:
day 37, control v 20d - (549.496 - 292.56) / 549.496 = 47% reduction

#summarize 304 mean and total activity data
Used this in the manuscript text
```{r}
activity_summary_304 <- master_304_outliers %>% 
  group_by(genotype, sex, age, treatment) %>% 
  summarise(
    mean_act = mean(daily_activity, na.rm = TRUE),
    total_act = sum(daily_activity, na.rm = TRUE),
...
activity_summary_304
```
For manuscript text:
304 males for day 51 vs day 26
5D - 59.00 to 239.09, 239.09/59 = 4.1x higher
C - 100.908 to 270.675, 270.675/100.908 = 2.7x higher
5DvC on day 11 - 169.81-117.31=52.5, 52.5/117.31 = 45% increase

#summarize 852 mean and total activity data
Used this in the manuscript text
```{r}
activity_summary_852 <- master_852_outliers %>% 
  group_by(genotype, sex, age, treatment) %>% 
  summarise(
    mean_act = mean(daily_activity, na.rm = TRUE),
    total_act = sum(daily_activity, na.rm = TRUE),
...
activity_summary_852
```
Day 37 females
Cvs5D - (457.468 - 245.423) / 245.423 = 86% higher
