...
```
#ultimate master boxplot for supplementary figure 1 with y-axis scaled to be the same for each genotype
```{r}
...
```
Sex, genotype, age, and treatment are significant, p<0.05
...
#ultimate master boxplot for supplementary figure 1, second version without y-axis scaled for each genotype
301
```{r}
boxplot_301 <- ggplot(data = master_301_outliers, aes(x = sex, y = daily_activity)) +
  geom_boxplot(aes(fill = "white", color = treatment), outlier.shape = NA, size = 0.75) +
  geom_jitter(aes(x = sex, y = daily_activity, color = treatment), 
              alpha = 0.6, 
              position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0), 
