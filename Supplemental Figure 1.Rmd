...
```
#combine all  timepoints to create ultimate_master
```{r}
ultimate_master <- rbind(master_t0, master_t1, master_t2, master_t3)
```

#plot ultimate master
```{r}
...
```
#subset into 5-day v 20-day data
```{r}
master_5day <- subset(ultimate_master, treatment %in% c("c", "5d"))

master_20day <- subset(ultimate_master, treatment %in% c("c", "20d"))
```
#subset further by age
```{r}
master_5day_11 <- subset(master_5day, age %in% c("11"))

master_5day_26 <- subset(master_5day, age %in% c("26"))
master_20day_26 <- subset(master_20day, age %in% c("26"))

master_5day_37 <- subset(master_5day, age %in% c("37"))
master_20day_37 <- subset(master_20day, age %in% c("37"))

master_5day_51 <- subset(master_5day, age %in% c("51"))
master_20day_51 <- subset(master_20day, age %in% c("51"))
```
##Ranked ANOVA, change data name
```{r}
master_20day_51$genotype <- as.factor(master_20day_51$genotype)
master_20day_51$sex <- as.factor(master_20day_51$sex)
master_20day_51$treatment <- as.factor(master_20day_51$treatment)

a <- master_20day_51[order(master_20day_51$daily_activity), ]
a$rank <- 1:nrow(a)
a <- aov(daily_activity ~ sex * genotype * treatment, data = a)
summary(a)
```
#Tukey HSD
```{r}
